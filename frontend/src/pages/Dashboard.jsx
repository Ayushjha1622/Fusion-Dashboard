...
export default function Dashboard() {
  ...
  return (
    <div className="flex h-screen bg-background text-on-surface font-body overflow-hidden">
      ...
      `}>
        {/* LOGO AREA */}
        <div className="p-8 border-b border-border border-opacity-30 flex justify-between items-center">
          ...
          {/* MOBILE CLOSE BUTTON */}
          <button 
            className="lg:hidden text-on-surface-dim hover:text-white"
            onClick={() => setIsSidebarOpen(false)}
          >
        ...
        <nav className="p-6 space-y-2 flex-1">
          ...
          <button
            onClick={() => { setActiveTab("sat"); setIsSidebarOpen(false); }}
      ...
      <main className="flex-1 flex flex-col relative overflow-hidden">
        ...
        <header className="h-16 glass-panel border-b border-border border-opacity-30 flex items-center justify-between px-4 lg:px-10 z-[2000]">
          <div className="flex items-center gap-4 lg:gap-8">
            {/* MOBILE MENU TOGGLE */}
            <button 
              className="lg:hidden text-primary p-1 hover:bg-primary/10 transition-colors"
            ...
            <div className="flex items-center gap-3">
              ...
              <span className="text-[10px] font-mono text-secondary font-bold tracking-widest hidden xs:inline">DEFCON: ELEVATED</span>
            </div>
            <div className="h-4 w-[1px] bg-border/40 hidden sm:block"></div>
            <div className="text-[10px] font-mono text-on-surface-dim hidden sm:flex gap-4 uppercase">
              <span>Sector: <span className="text-white">7-ALPHA</span></span>
              <span className="hidden md:inline">Grid: <span className="text-white">XV-902</span></span>
            </div>
          </div>
          ...
          <div className="flex items-center gap-4 lg:gap-6">
            ...
            <div className="flex gap-4 text-on-surface-dim">
              <span className="material-symbols-outlined hover:text-primary cursor-pointer transition-colors text-xl">notifications</span>
              <span className="material-symbols-outlined hover:text-primary cursor-pointer transition-colors text-xl">settings</span>
            </div>
          </div>
        </header>
